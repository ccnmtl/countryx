...
class GameTestCases(TestCase):
    ...
    def test_game_nochoices(self):
        ...
        self.assertEquals(
            your_player['model'],
            SectionGroupPlayer.objects.get(user=user, group=group))
    ...
    def test_game_decisionpending(self):
        ...
        group = SectionGroup.objects.get(
            name='A',
            section=Section.objects.get(name='Test'))
        current_state = group.sectiongroupstate_set.order_by(
            'date_updated')[0].state
        ...
        self.assertEquals(
            your_player['saved_choice'],
            StateRoleChoice.objects.get(state=current_state,
                                        role=player.role, choice=turn.choice))
    ...
    def test_game_finalsubmit(self):
        ...
        your_player = ctx.get('you')
        self.assertEquals(your_player['model'], player)
        self.assertEquals(your_player['submit_status'],
                          PLAYER_STATUS_SUBMITTED)
    ...
    def test_ajax_not_loggedin(self):
        c = self.client
        response = c.post('/sim/player/choose/',
                          {'groupid': 1, 'choiceid': 1,
                           'final': 0, 'reasoning': ''})
    ...
    def test_choose_savedraft_invalid(self):
        ...
        payload = ("<?xml version='1.0' encoding='utf-8'?><library><book>"
                   "<title>Blink</title><author>Malcolm Gladwell</author>"
                   "</book></library>")
